...
const rect = `M0,0 L1,0 L1,1 L0,1 Z`;

interface MorphProps {
  buttonBackground: React.ReactNode;
...
export default function Morph(props: MorphProps) {
  const {
    buttonBackground,
    children,
    duration = 10,
  ...
  const getLinearKeySplines = useCallback(() => {
          ...
          <clipPath id="morph-clip-path-1" clipPathUnits="objectBoundingBox">
            <path d={paths[0]}>
              <animate
                ...
                keySplines={getLinearKeySplines()}
                dur={`${duration}s`}
